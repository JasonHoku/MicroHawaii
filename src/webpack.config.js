...
var settings = {
  ...
  publicPath:
    "http://" + (argv.host || "localhost") + ":" + (argv.port || 8080) + "/",
...
function buildConfig(options) {
  ...
  var outputFileName = options.isDevelopment ? "[name].js" : "[name].[hash].js";
  ...
  return {
    ...
    entry: {
      ...
      index: [path.join(options.srcPath, "index")],
    },
    ...
    plugins: [
      ...
      options.isDevelopment ? new webpack.NoErrorsPlugin() : null,
      // Adding code minification.
      options.isDevelopment ? null : new webpack.optimize.UglifyJsPlugin(),
      // Creating additional gzipped file for each bundle.
      options.isDevelopment ? null : new CompressionPlugin(),
      // Removing duplicated code.
      options.isDevelopment ? null : new webpack.optimize.DedupePlugin(),
      // Creating a hash file holding the assets fingerprint.
      options.isDevelopment ? null : new HashPlugin({ path: outputPath }),
      // Enabling access to bower components.
      new BowerWebpackPlugin(),
      // Enabling I18n translations.
      options.locale
        ? new I18nPlugin(
            require(path.join(options.localeRootPath, options.locale + ".json"))
          )
        : null,
    ].filter(Boolean),
